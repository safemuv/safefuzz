...
bool CMOOSDB::OnRxPktCallBack(const std::string &sWho, MOOSMSG_LIST &MsgListRx,
                              MOOSMSG_LIST &MsgListTx, void *pParam) {
  CMOOSDB *pMe = (CMOOSDB *)(pParam);

  return pMe->OnRxPkt(sWho, MsgListRx, MsgListTx);
}

bool CMOOSDB::OnFetchAllMailCallBack(const std::string &sWho,
                                     MOOSMSG_LIST &MsgListTx, void *pParam) {
  CMOOSDB *pMe = (CMOOSDB *)(pParam);
...
void PrintHelpAndExit() {
  ...
  std::cout << "--moos_file=<string>               specify mission file name "
               "(default mission.moos)\n";
  std::cout << "--moos_port=<positive_integer>     specify server port number "
               "(default 9000)\n";
  std::cout << "--moos_time_warp=<positive_float>  specify time warp\n";
  std::cout << "--moos_community=<string>          specify community name\n";
  std::cout
      << "--moos_print_version               print build and version details\n";
  std::cout << "--moos_suicide_channel=<str>       suicide monitoring channel "
               "(IP address) \n";
  std::cout << "--moos_suicide_port=<int>          suicide monitoring port  \n";
  std::cout << "--moos_suicide_phrase=<str>        suicide pass phrase  \n";
  std::cout
      << "--moos_suicide_disable             disable suicide monitoring \n";
  std::cout << "--moos_suicide_print               print suicide conditions \n";
  std::cout
      << "--moos_no_colour                   dont use colour in printing \n";

  std::cout << "\nDB Control:\n";

  std::cout << "-d    (--dns)                      run with dns lookup\n";
  std::cout << "-s    (--single_threaded)          run as a single thread "
               "(legacy mode)\n";
  std::cout << "-b    (--moos_boost)               boost priority of "
               "communications\n";
  std::cout << "--moos_timeout=<positive_float>    specify client timeout\n";
  std::cout << "--response=<string-list>           specify tolerable client "
               "latencies in ms\n";
  std::cout << "--warning_latency=<positive_float>    specify latency above "
               "which warning is issued in ms\n";
  std::cout << "--tcpnodelay                       disable nagle algorithm \n";
  std::cout << "--audit_port=<unsigned int>        specify port on which to "
               "transmit statistics\n";
  std::cout << "--event_log=<file name>            specify file in which to "
               "record events\n";
  std::cout << "--print_heart_beat                 indicate DB heartbeat every "
               "second\n";
  ...
  std::cout << "  ./MOOSDB --moos_port=9001 "
               "--response=x_app:20,y_app:100,*_instrument:0\n";
  exit(0);
}
...
bool CMOOSDB::Run(int argc, char *argv[]) {
  ...
  // if either the mission file or command line have set a webserver port then
  // launch one
  if (nWebServerPort > 0)
  ...
  if (bSingleThreaded) {
    std::cout << MOOS::ConsoleColours::yellow()
              << "warning : running in single threaded mode performance will "
                 "be affected by poor networks\n"
              << MOOS::ConsoleColours::reset();
    m_pCommServer.reset(new CMOOSCommServer);
  } else {
    // std::cerr<<MOOS::ConsoleColours::green()<<"running in multi-threaded
    // mode\n"<<MOOS::ConsoleColours::reset();
...
template <class T>
void PrintCollection(const T &collection, ostream &out,
...
bool CMOOSDB::OnRxPkt(const std::string &sClient, MOOSMSG_LIST &MsgListRx,
  ...
  if (!MsgListRx.empty()) {
    ...
    if (q != m_HeldMailMap.end()) {
      // MOOSTrace("%f OnRxPkt %d messages held for client
      // %s\n",MOOSTime(),q->second.size(),sClient.c_str());

      if (!q->second.empty()) {
        // copy all the held mail to MsgListTx
        MsgListTx.splice(MsgListTx.begin(), q->second, q->second.begin(),
                         q->second.end());
      }
    }
  }
...
bool CMOOSDB::faultInEffect(CMOOSDBVar &rVar) {
  cout << "CMOOSDB faultInEffect = false" << endl;
  return false;
}
...
bool CMOOSDB::OnNotify(CMOOSMsg &Msg) {
  ...
  if (rVar.m_cDataType == Msg.m_cDataType) {
      ...
      if (!(this->faultInEffect(rVar))) {
          ...
          // record sSrc as a writer of this data, increment number of messages
          ...
          if (dfDT > 0.5) {
              // this looks a little hookey - the numbers are arbitrary to give
              // sensible looking frequencies when timing is coarse
              if (dfDT > 10.0) {
                  // MIN
      ...
      } else MOOSTrace("Update to variable ignored as fault was in effect");
      } else {
          MOOSTrace("Attempting to update var \"%s\" which is type %c with data "
                    "type %c by client %s (=%s)\n",
                    Msg.m_sKey.c_str(), rVar.m_cDataType, Msg.m_cDataType,
                    Msg.m_sSrc.c_str(), Msg.GetAsString(20, 4).c_str());
      }
      return true;
  }

  /** we now want to store some message in anoth cleints message box, when they
  next call
  in they shall be informed of the change by stuffing this msg into a return
  packet */
  ...
  bool CMOOSDB::OnRegister(CMOOSMsg & Msg) {
    ...
    if (Msg.IsType(MOOS_REGISTER)) {
      ...
      std::string detail =
          MOOSFormat("%s@%.1f", rVar.m_sName.c_str(), dfActualPeriod);
      m_EventLogger.AddEvent("register", Msg.m_sSrc, detail);
      ...
      for (q = m_VarMap.begin(); q != m_VarMap.end(); ++q) {
        ...
        if (F.Matches(M)) {
          ...
          if (!m_bQuiet) {
            std::cout << MOOS::ConsoleColours::yellow() << "+ subs of \""
  ...
  */
  ...
  bool CMOOSDB::OnConnect(string & sClient) {
    ...
    // notify ourselves....
    CMOOSMsg DBC(MOOS_NOTIFY, "DB_EVENT",
                 MOOSFormat("connected=%s", sClient.c_str()));
    DBC.m_sOriginatingCommunity = m_sCommunityName;
    DBC.m_sSrc = m_sDBName;
    OnNotify(DBC);
  ...
  bool CMOOSDB::OnDisconnect(string & sClient) {
    ...
    if (!m_bQuiet) {
      std::cout << MOOS::ConsoleColours::yellow() << sClient
                << " is leaving...           ";
    }
    ...
    // notify ourselves....
    CMOOSMsg DBC(MOOS_NOTIFY, "DB_EVENT",
                 MOOSFormat("disconnected=%s", sClient.c_str()));
    DBC.m_sOriginatingCommunity = m_sCommunityName;
    DBC.m_sSrc = m_sDBName;
    OnNotify(DBC);
  ...
  void CMOOSDB::UpdateSummaryVar() {
    ...
    for (p = m_VarMap.begin(); p != m_VarMap.end(); ++p) {
      ...
      // write frequency
      ss << std::fixed << std::setw(4) << std::setprecision(1)
         << p->second.m_dfWriteFreq << "Hz ";
  ...
  bool CMOOSDB::OnClearRequested(CMOOSMsg & Msg, MOOSMSG_LIST & MsgTxList) {
    ...
    MOOSTrace("    Removing %d existing notification queues...",
              m_HeldMailMap.size());
