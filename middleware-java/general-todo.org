...
The CI requests must be passed as serialised Behaviour objects (subclasses) to the middleware
The middleware has to have an ATLASEventQueue monitoring communications from the CI
Then relay out the corresponding MOOS variable updates from the CI into the DB
Finish the conversion of these in the middleware - in CIEventQueue.handleEvent
Types e.g. CIEvent may need to be handled
