...
* TODO Register additional objects
* TODO ActiveMQ port conversion - the link from the CI to the application should be an
ActiveMQ port named the name of the relevant vehicle? Or by the CI location?

If it is the CI location, then it is necessary for the application to provdie 
