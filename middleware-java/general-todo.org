...
** TODO Need middleware set up with speed and heading parameters for all robots - so they can be restored properly after fault
*** TODO Logging - the mission completion time,  detection timings for objects
** DONE Need to specify the fault instance filename to the middleware
   CLOSED: [2020-04-22 Wed 01:36]
** TODO How to start the MOOS simulation automatically with proper timing control? - without having to click DEPLOY - change generated MOOS behaviours
