...
def parallel_coordinate_plot(data_filename, title, output_image):
    ...
    ynames = ['OuterRgV', 'InnerRgV', 'AvoidV', 'SpeedV', 'TimeLength']
    data_rows = df.shape[0]
    N = data_rows
    category = np.arange(data_rows)
#    print(category)
    y1 = df["OutsideOfOuterRegionViolations"]
    y2 = df["OutsideOfInnerRegionViolations"]
    y3 = df["AvoidanceViolation"]
    y4 = df["SpeedViolationsCount"]
    y5 = df["FuzzingTimeLength"]
